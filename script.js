// DOCUMENT ELEMENTS
let clockScreen = document.getElementById("clock-screen");
...
// Function
function createSpecificElement(tag, elementClass, textContent = "") {
...
// CLASSES
function Clock(id = "clock") {
    this.element = document.getElementById(id);

    this.interval = setInterval(() => {
        let date = luxon.DateTime.now().toFormat("HH:mm");
...
function Timer(id = "timer") {
    ...
    const createTimerInterval = (timerClock) => {
    ...
    this.element.onclick = () => {
        // Create elements
        let timerScreen = createSpecificElement("div", "function-screen");
        let stopButton = createSpecificElement("button", false, "Stop");
        let timerClock = createSpecificElement(
            "div",
            "function-text",
            "00:00:00"
        );
        ...
        let interval = createTimerInterval(timerClock);
        ...
        stopButton.onclick = () => {
            ...
            stopButton.addEventListener("click", () => {
                clockScreen.removeChild(timerScreen);
            });
        };

        // Append child to clock screen
        timerScreen.appendChild(timerClock);
        timerScreen.appendChild(stopButton);
        clockScreen.appendChild(timerScreen);
    };
}
...
function clickCountdown() {
    ...
    countdownDiv.classList.add("function-screen");
    ...
    countdownButton.addEventListener("click", () => {
        ...
        countdownTimer.classList.add("function-text");
        ...
        let interval = setInterval(() => {
            ...
            if (countdownValue != "00:00:00") {
                let time = luxon.DateTime.fromFormat(
                    countdownValue,
                    "HH:mm:ss"
                );
...
function clickAlarm() {
    ...
    alarmDiv.classList.add("function-screen");
...
function checkAlarms() {
    ...
    for (let alarmItem of alarms)
        if (alarmItem.textContent == clock.textContent) return alarmItem;
